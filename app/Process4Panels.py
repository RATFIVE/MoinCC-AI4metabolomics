...
class Process4Panels:
    ...
    def save_sum_spectra(self):
        ...
        for i, row in enumerate(self.fitting_params.iterrows()):
            ...
            positions = row[1][1:n_peaks+1]
            widths = row[1][1+n_peaks: 2*n_peaks+1]
            amplitudes = row[1][2*n_peaks+1:]
            y = np.zeros(len(x))
            for position,width, amplitude in zip(positions, widths, amplitudes):
                y += self.lorentzian(x, position, width, amplitude)
    ...
    def save_substrate_individual(self):
        ...
        for i, row in enumerate(self.fitting_params.iterrows()):
            ...
            widths = row[1].iloc[1 + n_peaks: 2 * n_peaks + 1]
    ...
    def save_kinetics(self):
        ...
        names = self.fitting_params.columns[1:]  # This remains the same because `index` is used for column labels
